...
def print_statistical_summary(df, title="Statistical Summary"):
    ...
    for (param, level), group in groups:
        print(f"\n>>> Condition: {param.upper()} | Level: {level.upper()}")
        print("-" * 80)
        ...
        best_mean = means[best_method_name]
...
# ============================================================
# 5. Evaluation Loop (UPDATED HUBER)
# ============================================================
...
def evaluate_huber(all_results, omega, Dll_samples, Dhl_samples, N_TRIALS):
    ...
    # --- UPDATED CONFIGS: 0.2 and 0.5 ---
    # format: (alpha, noise_scale, level_name)
    configs = [
        (0.0, 0.0, "clean"),
        (1.0, 0.2, "noisy_0.2"),
        (1.0, 0.5, "noisy_0.5")
    ...
    count = sum(len(fd) for folds in all_results.values() if isinstance(folds, dict) for fd in folds.values() if isinstance(fd, dict))
    pbar = tqdm(total=count * len(configs) * N_TRIALS, desc="Evaluating Huber")
                    ...
                    for t in range(N_TRIALS):
                        try:
                            ...
                            for iota, eta in omega.items():
                                ...
                                seed = hash((fold_idx, "huber", level_name, t, iota)) % (2**32)
                                
                                # Use the specific scale from config
                            ...
                            records.append({
                                "method": m_name, "param": "huber", "level": level_name, 
                                "trial": t, "fold": fold_idx, "rel_l2": avg
                            })
                        except: pass
                        pbar.update(1)
...
def generate_visuals(Dll_samples, levels_dict, num_digits=5):
    ...
    # --- UPDATED VISUALS: Huber 0.2 and 0.5 ---
    # Huber 0.2
...
def make_pdf_report(df_cam, df_hub, vis_rows, vis_labels, out_pdf):
    with PdfPages(out_pdf) as pdf:
        ...
        for df, name in [(df_cam, "Camera"), (df_hub, "Huber")]:
            if df.empty: continue
            for p in df["param"].unique():
                df_p = df[df["param"] == p]
                fig, ax = plt.subplots(figsize=(10, 6))
                
                # Dynamic order for Huber
                hue_order = None
                if p == "huber":
                    hue_order = sorted(df_p["level"].unique()) # clean, noisy_0.2, noisy_0.5
                
                sns.boxplot(data=df_p, x="method", y="rel_l2", hue="level", hue_order=hue_order, palette="pastel", showfliers=False, ax=ax)
...
def main():
    ...
    print("Evaluating Huber (0.0 / 0.2 / 0.5)...")
    df_hub = evaluate_huber(res, om, dll, dhl, args.n_trials)
    
    print_statistical_summary(df_cam, "Camera Shift Stats")
    print_statistical_summary(df_hub, "Huber Shift Stats")
    
    vis_rows, vis_lbls = generate_visuals(dll, levels_dict)
    make_pdf_report(df_cam, df_hub, vis_rows, vis_lbls, "cmnist_results_huber_extended.pdf")
