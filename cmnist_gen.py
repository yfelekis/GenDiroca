#!/usr/bin/env python3
"""
ColorMNIST Data Generator (parameterized + optional photometric & sensor noise)

Key points:
- Pass all experiment + noise params via function args or CLI flags.
- Resolution is fully modular (any square size).
- Noise and jitters are applied in [0,1] and gated by NOISE_ENABLE.
"""
...
class ColorMNISTDataGenerator:
    ...
    def generate_samples(self, n: int, intervention: Optional[Intervention] = None):
        ...
        - final_images: (N,3,H,W) in [0,1]
        - img_shapes: (N,1,H,W) in [-1,1]
...
def low_to_high_level(ll_samples_tuple: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
    """
    Map low-level tensors to the high level:
    - digits/colors -> one-hot
    - image_feature = mean intensity of colored image (on [0,1])
    Returns (N, 21): [10 digit one-hots | 10 color one-hots | 1 feature]
    """
    final_images, _, digits, colors = ll_samples_tuple
    image_feature = final_images.mean(dim=[1, 2, 3]).unsqueeze(1)  # (N,1) on [0,1]
    ...
    return torch.cat([digits_onehot, colors_onehot, image_feature], dim=1)  # (N,21)
...
def generate_cmnist_data(
    ...
    Saves:
      ...
      - dhl_samples.pkl : dict[str] -> (N,21) HL tensor
      - intervention_mapping.pkl : dict[str,str] omega_labels (LL label -> HL label)
      - intervention_mapping.json : same, JSON for readability
    """
    ...
    print("\nGenerating high-level data by applying the abstraction...")
    Dhl_samples: Dict[str, torch.Tensor] = {}
    for lab in tqdm(INTERVENTION_LABELS, desc="Generating High-Level Data"):
        Dhl_samples[lab] = low_to_high_level(Dll_samples[lab])
...
def build_argparser():
    p = argparse.ArgumentParser(description="Generate Colored MNIST with configurable resolution, correlation, and noise.")
    p.add_argument("--resolution", type=int, default=32, help="Output image resolution (HxW).")
...
def main():
    ...
    print("ColorMNIST Data Generator (parameterized)")
    print("=" * 50)
