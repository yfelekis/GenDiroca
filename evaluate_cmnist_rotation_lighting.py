...
# ============================================================
# 4. Statistical Analysis (FIXED PIVOT)
# ============================================================
def print_statistical_summary(df, title="Statistical Summary"):
    print(f"\n{'='*20} {title} {'='*20}")
    
    # Create a unique ID for every sample to handle multiple folds
    # This prevents the "duplicate entries" error
    ...
    for (param, level), group in groups:
        ...
        # PIVOT using the new unique sample_id
...
# ============================================================
# 5. Evaluation Loop (UPDATED SEEDS)
# ============================================================
def method_display_name(g, r):
    if g.startswith("diroca"): return f"DiRoCA ({r})"
    if g == "abslingam": return r
                        ...
                        for t in range(N_TRIALS):
                            try:
                                ...
                                for iota, eta in omega.items():
                                    ...
                                    # --- KEY CHANGE: Removed 'rk' from seed ---
                                    # This ensures all methods see the exact same transformed image for trial 't'
                                ...
                                # Added fold_idx for unique indexing
                                records.append({
                    ...
                    for t in range(N_TRIALS):
                        try:
                            ...
                            for iota, eta in omega.items():
                                ...
                                # --- KEY CHANGE: Removed 'rk' from seed ---
                                seed = hash((fold_idx, "huber", alpha, t, iota)) % (2**32)
...
def main():
    ...
    vis_rows, vis_lbls = generate_visuals(dll, levels_dict)
    make_pdf_report(df_cam, df_hub, vis_rows, vis_lbls, "cmnist_results_significance.pdf")
