...
# ============================================================
# 2. Transforms (Rotation, Blur, Huber)
# ============================================================
# def build_camera_transform(rotation_deg, blur_sigma):
#     transforms = []
#     # 1. Rotation (fill=-1 for black background)
#     if rotation_deg != 0:
...
# ============================================================
# 2. Transforms (FIXED: Deterministic Rotation)
# ============================================================
def build_camera_transform(rotation_deg, blur_sigma):
    transforms = []
    ...
    # 1. Rotation (fill=-1 for black background)
    if rotation_deg != 0:
        # CHANGE: Pass a tuple (deg, deg) to force a fixed rotation
        # Old: degrees=rotation_deg -> Range [-deg, +deg] (Can be 0)
        # New: degrees=(rotation_deg, rotation_deg) -> Exactly deg
...
# ============================================================
# 4. Evaluation Loop (Camera + Huber)
# ============================================================
...
def evaluate_main(all_results, omega, Dll_samples, Dhl_samples, N_TRIALS, levels_dict):
    records = []
    # ONLY Rotation and Blur
    param_names = ["rotation", "blur"]
    levels = ["low", "high"]
            ...
            for rk, res in fd.items():
                ...
                for p in param_names:
                    for l in levels:
                        ...
                        bl = levels_dict["blur"][l] if p=="blur" else 0
                        
                        tf = build_camera_transform(rot, bl)
...
def make_pdf_report(df_cam, df_hub, vis_rows, vis_labels, out_pdf):
    with PdfPages(out_pdf) as pdf:
        ...
        plt.tight_layout()
        fig.suptitle("CMNIST Shift Visuals (Rot/Blur/Huber)", y=1.01, fontsize=16)
        ...
        if not df_hub.empty:
            ...
            ax.set_title("Huber Contamination (Affine Corrected)")
...
def main():
    ...
    # --- UPDATED LEVELS ---
    levels_dict = {
        "rotation": {"low": 20.0, "high": 90.0},
        "blur": {"low": 1.0, "high": 5.0}
    }
    
    print("Evaluating Rotation & Blur...")
    ...
    make_pdf_report(df_cam, df_hub, vis_rows, vis_lbls, "cmnist_paper_results.pdf")
